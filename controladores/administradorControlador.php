	...
	class administradorControlador extends mainModel{
        ...
        public function paginador_administrador_controlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE ((usuario_id!='$id' AND usuario_id!='".$_SESSION['id_database']."') AND (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' OR usuario_cargo LIKE '%$busqueda%' OR usuario_usuario LIKE '%$busqueda%')) ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
			}else{
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE usuario_id!='".$_SESSION['id_database']."' AND usuario_id!='1' ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
			}
		...
		public function actualizar_administrador_controlador(){
			...
			if($id==1 && $_SESSION['id_database']!=$id){
				$alerta=[
                    "Alerta"=>"simple",
                    "Titulo"=>"Acceso no permitido",
                    "Texto"=>"No tienes los permisos necesarios para realizar esta operación en el sistema",
			...
			if(mainModel::actualizar_datos("usuario",$datos_usuario_up,$condicion)){
				...
				if($_SESSION['id_database']==$id){
					$_SESSION['nombre_database']=$nombre;
					$_SESSION['apellido_database']=$apellido;
					$_SESSION['genero_database']=$genero;
					$_SESSION['usuario_database']=$usuario;
					$_SESSION['cargo_database']=$cargo;
					$_SESSION['foto_database']=$avatar;
				}
